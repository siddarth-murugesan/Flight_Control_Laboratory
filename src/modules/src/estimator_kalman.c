  ...
  while (estimatorDequeue(&m)) {
    switch (m.type) {
      ...
      case MeasurementTypeTOF:
       if(useFAndR){
          // Standard Tof update added with obstacle detection below the drone
      ...
      case MeasurementTypeUpTOF:
        if(useFAndR){
          // Multiranger deck tof update of the obstacle detection above the drone
      ...
      case MeasurementTypeFlow:
        if(useFAndR){
          // Modified flow update for obstacle detection
